...
ORIGINAL MESSAGE:
{original.text or '[Media/Photo]'}

REPLY:
{message.text or '[Media/Photo]'}
═══════════════════════
"""
        send_pe(OWNER_ID, forward_text, use_premium=True, use_bold=True)
...
REPLIED BY: {replier_name}
USERNAME: {replier_username}
REPLY: {message.text or '[Media/Photo]'}
═══════════════════════
""", use_premium=True, use_bold=True)
                except:
                    pass
    except Exception as e:
...
def get_admin_menu(uid):
    ...
    markup.row(KeyboardButton(stylish_text("START ADS")), KeyboardButton(stylish_text("STOP ADS")))
    markup.row(KeyboardButton(stylish_text("SET INTERVAL")), KeyboardButton(stylish_text("SET POSTS PER CYCLE")))
    ...
    markup.row(KeyboardButton(stylish_text("SET HOW TO USE")), KeyboardButton(stylish_text("CHANGE WELCOME IMAGE")))
    markup.row(KeyboardButton(stylish_text("BOT ON")), KeyboardButton(stylish_text("BOT OFF")))
    markup.row(KeyboardButton(stylish_text("STATUS")), KeyboardButton(stylish_text("HELP")))
    markup.row(KeyboardButton(stylish_text("ABOUT")), KeyboardButton(stylish_text("")))
    return markup
...
"""
        ...
        send_pe(message.chat.id, final_text, reply_markup=markup, use_premium=True, use_bold=True)
    except Exception as e:
...
def start_ads_cmd(message):
    ...
    if not user.get("approved", False) and not is_admin(uid):
        send_pe(message.chat.id, "Buy subscription first!", use_premium=True, use_bold=True)
        return
    ...
    if sending_active:
        send_pe(message.chat.id, "Ads already running!", use_premium=True, use_bold=True)
        return
...
def stop_ads_cmd(message):
    ...
    if not is_admin(uid):
        ...
        if not user or not user.get("approved", False):
            send_pe(message.chat.id, "You are not approved!", use_premium=True, use_bold=True)
            return
...
# ============================================================
# ALL OTHER COMMANDS - SHORT VERSION
# ============================================================
...
"""
    ...
    try:
        bot.send_photo(message.chat.id, photo=qr_url, caption=final_text, reply_markup=markup)
    except:
        send_pe(message.chat.id, final_text, reply_markup=markup, use_premium=True, use_bold=True)
...
def receive_payment_screenshot(message):
    ...
    if message.photo:
        ...
        send_pe(message.chat.id, "Received! Waiting for admin.", use_premium=True, use_bold=True)
...
"""
        ...
        for admin in ADMIN_IDS:
            try:
                bot.send_photo(admin, photo=file_id, caption=final_admin, reply_markup=markup)
            except:
                send_pe(admin, final_admin, reply_markup=markup, use_premium=True, use_bold=True)
    else:
        send_pe(message.chat.id, "Send a photo!", use_premium=True, use_bold=True)
...
"""
    ...
    send_pe(message.chat.id, final_text, use_premium=True, use_bold=True)
...
def process_set_posts_per_cycle(message):
    ...
    try:
        ...
        if num < 5 or num > 15:
            send_pe(message.chat.id, "Min: 5 | Max: 15", use_premium=True, use_bold=True)
            return
        settings = load_settings()
        settings["posts_per_cycle"] = num
        save_settings(settings)
        send_pe(message.chat.id, f"Posts per cycle set to {num}!", use_premium=True, use_bold=True)
    except:
...
def process_delete_post(message):
    ...
    try:
        post_num = int(message.text.strip())
        posts = load_posts()
        if post_num < 1 or post_num > len(posts):
            send_pe(message.chat.id, "Invalid post number!", use_premium=True, use_bold=True)
            return
        
        deleted = posts.pop(post_num - 1)
        save_posts(posts)
        
        text = f"""
POST DELETED
═══════════════════════
Post #{post_num} deleted
═══════════════════════
"""
        final_text = ""
        for line in text.split('\n'):
        ...
        send_pe(message.chat.id, "Invalid input!", use_premium=True, use_bold=True)
...
def process_change_welcome_image(message):
    ...
    if message.photo:
        ...
        save_settings(settings)
        send_pe(message.chat.id, "Welcome image URL updated!", use_premium=True, use_bold=True)
    else:
        send_pe(message.chat.id, "Send a photo or URL!", use_premium=True, use_bold=True)

# ============================================================
# OTHER ADMIN COMMANDS - QUICK
# ============================================================
...
def premium_on(message):
    ...
    save_settings(settings)
    send_pe(message.chat.id, "Premium Emojis ON!", use_premium=True, use_bold=True)
...
def premium_off(message):
    ...
    save_settings(settings)
    send_pe(message.chat.id, "Premium Emojis OFF!", use_premium=False, use_bold=True)
...
def bold_on(message):
    ...
    save_settings(settings)
    send_pe(message.chat.id, "Bold Characters ON!", use_premium=True, use_bold=True)
...
def bold_off(message):
    ...
    save_settings(settings)
    send_pe(message.chat.id, "Bold Characters OFF!", use_premium=True, use_bold=False)
...
def process_add_group(message):
    ...
    if group_id in groups:
        send_pe(message.chat.id, "Group already added!", use_premium=True, use_bold=True)
        return
...
def addgroup_cmd(message):
    ...
    if message.chat.type in ['group', 'supergroup']:
        ...
        send_pe(message.chat.id, "Only works in groups!", use_premium=True, use_bold=True)
...
def process_remove_group(message):
    ...
    del groups[group_id]
    save_groups(groups)
    send_pe(message.chat.id, f"Group removed!", use_premium=True, use_bold=True)
...
SET INTERVAL
═══════════════════════
Current: {settings.get('send_interval', 6)} sec
Min: 2 | Max: 600
Send new interval:
═══════════════════════
"""
...
def process_set_interval(message):
    ...
    try:
        ...
        if interval < 2 or interval > 600:
            send_pe(message.chat.id, "Min: 2 | Max: 600", use_premium=True, use_bold=True)
            return
        settings = load_settings()
        settings["send_interval"] = interval
        save_settings(settings)
        send_pe(message.chat.id, f"Interval set to {interval}s!", use_premium=True, use_bold=True)
    except:
        send_pe(message.chat.id, "Invalid number!", use_premium=True, use_bold=True)
...
def users_cmd(message):
    ...
    users = load_users()
    if not users:
        send_pe(message.chat.id, "No users!", use_premium=True, use_bold=True)
        return
    text = "USERS\n═══════════════════════\n"
...
def total_messages_cmd(message):
    if not is_admin(message.from_user.id):
        return
    stats = load_stats()
    send_pe(message.chat.id, f"Total Messages: {stats.get('total_messages', 0)}", use_premium=True, use_bold=True)
...
def process_change_price(message):
    ...
    try:
        ...
        if price <= 0:
            send_pe(message.chat.id, "Price > 0 required!", use_premium=True, use_bold=True)
            return
        settings = load_settings()
...
def process_set_how_to_use(message):
    ...
    if message.video:
        ...
        save_settings(settings)
        send_pe(message.chat.id, "How to use video updated!", use_premium=True, use_bold=True)
    elif message.text:
        settings["how_to_use_text"] = message.text.strip()
        settings["how_to_use_video"] = None
        save_settings(settings)
        send_pe(message.chat.id, "How to use text updated!", use_premium=True, use_bold=True)
...
def how_to_use(message):
    ...
    if video:
        try:
            bot.send_video(message.chat.id, video=video, caption=final_text)
        except:
            send_pe(message.chat.id, final_text, use_premium=True, use_bold=True)
    else:
        send_pe(message.chat.id, final_text, use_premium=True, use_bold=True)
...
"""
                    ...
                    if welcome_image:
                        try:
                            bot.send_photo(message.chat.id, photo=welcome_image, caption=final_text)
                        except:
                            send_pe(message.chat.id, final_text, use_premium=True, use_bold=True)
                    else:
                        send_pe(message.chat.id, final_text, use_premium=True, use_bold=True)
