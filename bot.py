...
def send_pe(chat_id, text, reply_markup=None, parse_mode=None):
    """Always sends with premium emojis + stylish"""
    try:
...
def send_to_group(group_id, text, photos=None, dm_username=None):
    ...
    try:
        ...
        if photos and len(photos) > 1:
            ...
            # Send text after photos
            send_pe(group_id, text, reply_markup=markup)
        elif photos and len(photos) == 1:
            bot.send_photo(group_id, photo=photos[0], caption=text, reply_markup=markup)
        else:
            send_pe(group_id, text, reply_markup=markup)
...
# ============================================================
# COLORFUL BUTTONS - NO PREMIUM EMOJIS ON BUTTONS
# ============================================================
...
def process_add_post(message):
    ...
    if message.media_group_id:
        # Multiple photos in album
        bot.register_next_step_handler_by_chat_id(message.chat.id, process_add_post)
        return
    
    if message.photo:
        photos.append(message.photo[-1].file_id)
        # Check for more photos in album
        if message.media_group_id:
            bot.register_next_step_handler(message, process_add_post)
            return
