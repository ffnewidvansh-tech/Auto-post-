...
def get_random_premium_emoji():
    # sirf wahi emojis chuno jinka fallback known hai (render guaranteed)
...
def utf16_len(text: str) -> int:
    """Telegram entities UTF-16 code units me count karte hain, Python len() emoji pe galat deta hai."""
...
def build_message(text: str):
    """Always adds stylish + premium emojis"""
...
def send_pe(chat_id, text, reply_markup=None, parse_mode=None):
    """Always sends with premium emojis + stylish (fallback bhi stylish hota hai)"""
    try:
        processed_text, entities = build_message(text)
        if parse_mode:
            # entities + parse_mode ek saath nahi chal sakte
        ...
        try:
            # premium emoji fail -> plain stylish text with emoji
...
def send_to_group(group_id, text, photos=None, dm_username=None):
    """Send to group with multiple photos support"""
    try:
...
def make_red_button(text, callback=None, url=None):
    # Defined hai, par code me kahi use nahi hota (sab green chahiye)
    final = stylish_text(text)
    try:
        if callback:
...
def get_dm_for_post(post, settings=None):
    """Har post ke owner ka DM username use karo, warna global wala"""
    if settings is None:
        settings = load_settings()
...
sending_active = False
sending_thread = None
album_buffers = {}  # media_group_id -> [messages]
...
def get_user_menu(uid):
    ...
    markup.row(KeyboardButton(stylish_text("HOW TO USE")), KeyboardButton(stylish_text("SUPPORT")))
...
# ============================================================
# ADD POST - MULTIPLE PHOTOS SUPPORT (users + admin)
# ============================================================
...
def process_add_post(message):
    ...
    # ALBUM - saare photos collect karo
...
# ============================================================
# ADD DM IN ADS - PER-USER DM (post ke niche blue DM ME button)
# ============================================================
...
def process_add_dm(message):
    ...
    # ab global settings me nahi, USER ke record me save hota hai
...
"""
    markup = InlineKeyboardMarkup([
        [make_green_button("CONTACT SUPPORT", url="https://t.me/iflexzyan")]
    ])
    send_pe(message.chat.id, text, reply_markup=markup)
